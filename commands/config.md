...
1. Read `.claude/caravel/settings.json`.
...
3. Also list any cron jobs from `.claude/caravel/jobs/` with their name and schedule.
...
2. Read `.claude/caravel/settings.json`.
...
1. Read `.claude/caravel/settings.json` and show the current prompt.
...
3. Read `.claude/caravel/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "Which Claude model should Caravel use?" (header: "Model", options: "opus (default)", "sonnet", "haiku", "glm")
3. Read `.claude/caravel/settings.json`.
...
1. If token is in `$ARGUMENTS`, use it directly.
2. Otherwise, use **AskUserQuestion**: "What API token should Caravel use for glm?" (header: "API token", options: let user type via Other)
3. Read `.claude/caravel/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "Which fallback model should Caravel use?" (header: "Fallback model", options: "glm (Recommended)", "sonnet", "haiku")
3. Read `.claude/caravel/settings.json`.
...
1. If token is in `$ARGUMENTS`, use it directly.
2. Otherwise, use **AskUserQuestion**: "What API token should Caravel use for fallback model?" (header: "Fallback API token", options: let user type via Other)
3. Read `.claude/caravel/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "What timezone should Caravel use?" (header: "Timezone", options: "UTC (Recommended)", "America/New_York", "Europe/London")
3. Read `.claude/caravel/settings.json`.
...
Location: `.claude/caravel/settings.json`
