...
**CRITICAL: Job files MUST be created in the project-relative path `.caravel/jobs/`, NOT in `~/.caravel/jobs/`.** The daemon only watches the project directory. Using the home directory path will silently fail — the job will never fire.
...
1. List all `.md` files in `.caravel/jobs/`.
...
3. Create the job file at `.caravel/jobs/<name>.md` with this exact format:
   ```markdown
   ---
...
1. Read `.caravel/jobs/<job-name>.md`. If it doesn't exist, list available jobs and ask the user which one to edit.
...
3. If confirmed, delete `.caravel/jobs/<job-name>.md`.
...
1. Read `.caravel/jobs/<job-name>.md`. If it doesn't exist, list available jobs.
...
Jobs live in `.caravel/jobs/` as markdown files:
